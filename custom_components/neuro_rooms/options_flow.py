...
from .rooms import build_advanced_room_payload, build_basic_room_payload, parse_json_payload
...
class NeuroRoomsOptionsFlow(OptionsFlowWithReload):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None):
        ...
        schema = vol.Schema(
            {
                vol.Required(CONF_AUTO_DISCOVERY, default=self._options.get(CONF_AUTO_DISCOVERY, self._data.get(CONF_AUTO_DISCOVERY, True))): selector.BooleanSelector(),
                vol.Required(CONF_DEFAULT_LANGUAGE, default=self._options.get(CONF_DEFAULT_LANGUAGE, self._data.get(CONF_DEFAULT_LANGUAGE, DEFAULT_LANGUAGE))): selector.SelectSelector(
    ...
    async def async_step_rooms_menu(self, user_input: dict[str, Any] | None = None):
        ...
        schema = vol.Schema(
            {
                vol.Required(CONF_ROOM_ACTION, default=ROOM_ACTION_ADD_BASIC): selector.SelectSelector(
                    selector.SelectSelectorConfig(
                        options=[ROOM_ACTION_ADD_BASIC, ROOM_ACTION_ADD_ADVANCED, ROOM_ACTION_EDIT_JSON, ROOM_ACTION_BACK],
                        mode=selector.SelectSelectorMode.DROPDOWN,
                    )
                ),
            }
        ...
        schema = vol.Schema(
            {
                ...
                vol.Required(CONF_ROOM_MODE, default=mode_options[0]): selector.SelectSelector(
                    selector.SelectSelectorConfig(options=mode_options, mode=selector.SelectSelectorMode.DROPDOWN)
                ),
                vol.Required(CONF_ROOM_MODIFIER, default=modifier_options[0]): selector.SelectSelector(
                    selector.SelectSelectorConfig(options=modifier_options, mode=selector.SelectSelectorMode.DROPDOWN)
                ),
                vol.Required(CONF_ROOM_HAS_LIGHTING, default=True): selector.BooleanSelector(),
                vol.Required(CONF_ROOM_HAS_HVAC, default=False): selector.BooleanSelector(),
                vol.Required(CONF_ROOM_HAS_MEDIA, default=False): selector.BooleanSelector(),
            }
        )
        ...
        if user_input is not None:
            try:
                ...
                return self.async_show_form(
                    ...
                    data_schema=self._room_advanced_schema(default=user_input.get(CONF_ROOM_ADVANCED_JSON, "")),
                    errors={CONF_ROOM_ADVANCED_JSON: "invalid_json"},
                )
            self._options[CONF_ROOMS] = self._rooms
            return await self.async_step_rooms_menu()

        return self.async_show_form(step_id="room_advanced", data_schema=self._room_advanced_schema())
    ...
    async def async_step_rooms_json(self, user_input: dict[str, Any] | None = None):
        if user_input is not None:
            try:
                ...
                return self.async_show_form(
                    step_id="rooms_json",
                    data_schema=self._rooms_json_schema(default=user_input.get(CONF_ADVANCED_ROOMS_JSON, "")),
                    errors={CONF_ADVANCED_ROOMS_JSON: "invalid_json"},
                )
            self._options[CONF_ROOMS] = self._rooms
            return await self.async_step_rooms_menu()

        return self.async_show_form(step_id="rooms_json", data_schema=self._rooms_json_schema())
    ...
    async def async_step_modes_menu(self, user_input: dict[str, Any] | None = None):
        ...
        schema = vol.Schema(
            {
                vol.Required(CONF_MODE_ACTION, default=MODE_ACTION_ADD_BASIC): selector.SelectSelector(
                    selector.SelectSelectorConfig(
                        options=[MODE_ACTION_ADD_BASIC, MODE_ACTION_ADD_ADVANCED, MODE_ACTION_EDIT_JSON, MODE_ACTION_BACK],
                        mode=selector.SelectSelectorMode.DROPDOWN,
                    )
                ),
            }
        ...
        schema = vol.Schema(
            {
                ...
                vol.Required(CONF_MODE_HAS_LIGHTING, default=True): selector.BooleanSelector(),
                vol.Required(CONF_MODE_HAS_HVAC, default=True): selector.BooleanSelector(),
                vol.Required(CONF_MODE_HAS_MEDIA, default=True): selector.BooleanSelector(),
            }
        )
        ...
        if user_input is not None:
            try:
                ...
                if not isinstance(payload, dict):
                    raise ValueError("mode json must be an object")
                self._modes.append(payload)
            except Exception:
                return self.async_show_form(
                    step_id="mode_advanced",
                    data_schema=self._mode_advanced_schema(default=user_input.get(CONF_MODE_ADVANCED_JSON, "")),
                    errors={CONF_MODE_ADVANCED_JSON: "invalid_json"},
                )
            self._options[CONF_MODES] = self._modes
            return await self.async_step_modes_menu()

        return self.async_show_form(step_id="mode_advanced", data_schema=self._mode_advanced_schema())
    ...
    async def async_step_modes_json(self, user_input: dict[str, Any] | None = None):
        if user_input is not None:
            try:
                ...
                return self.async_show_form(
                    step_id="modes_json",
                    data_schema=self._modes_json_schema(default=user_input.get(CONF_ADVANCED_ROOMS_JSON, "")),
                    errors={CONF_ADVANCED_ROOMS_JSON: "invalid_json"},
                )
            parsed = parse_json_payload(user_input[CONF_ADVANCED_ROOMS_JSON])
            if parsed is None:
        ...
        return self.async_show_form(step_id="modes_json", data_schema=self._modes_json_schema())
    ...
    async def async_step_modifiers_menu(self, user_input: dict[str, Any] | None = None):
        ...
        schema = vol.Schema(
            {
                vol.Required(CONF_MODIFIER_ACTION, default=MODIFIER_ACTION_ADD_BASIC): selector.SelectSelector(
                    selector.SelectSelectorConfig(
                        options=[MODIFIER_ACTION_ADD_BASIC, MODIFIER_ACTION_ADD_ADVANCED, MODIFIER_ACTION_EDIT_JSON, MODIFIER_ACTION_BACK],
                        mode=selector.SelectSelectorMode.DROPDOWN,
                    )
                ),
            }
            ...
            self._modifiers.append(
                {
                    ...
                    "overrides": json.loads(user_input[CONF_MODIFIER_OVERRIDES_JSON]) if user_input[CONF_MODIFIER_OVERRIDES_JSON].strip() else {},
                    "kind": "basic",
                }
            )
        ...
        schema = vol.Schema(
            {
                ...
                vol.Required(CONF_MODIFIER_BASE_MODE, default=mode_options[0]): selector.SelectSelector(
                    selector.SelectSelectorConfig(options=mode_options, mode=selector.SelectSelectorMode.DROPDOWN)
                ),
                vol.Required(CONF_MODIFIER_DESCRIPTION): str,
                vol.Optional(CONF_MODIFIER_OVERRIDES_JSON, default="{}"): selector.TextSelector(
                    selector.TextSelectorConfig(type=selector.TextSelectorType.TEXT, multiline=True)
                ),
            }
        )
        return self.async_show_form(step_id="modifier_basic", data_schema=schema)

    async def async_step_modifier_advanced(self, user_input: dict[str, Any] | None = None):
        if user_input is not None:
            try:
                payload = parse_json_payload(user_input[CONF_MODIFIER_ADVANCED_JSON])
                if not isinstance(payload, dict):
                    raise ValueError("modifier json must be an object")
                self._modifiers.append(payload)
            except Exception:
                return self.async_show_form(
                    step_id="modifier_advanced",
                    data_schema=self._modifier_advanced_schema(default=user_input.get(CONF_MODIFIER_ADVANCED_JSON, "")),
                    errors={CONF_MODIFIER_ADVANCED_JSON: "invalid_json"},
                )
            self._options[CONF_MODIFIERS] = self._modifiers
            return await self.async_step_modifiers_menu()

        return self.async_show_form(step_id="modifier_advanced", data_schema=self._modifier_advanced_schema())
        ...
        if user_input is not None:
            try:
                ...
                return self.async_show_form(
                    ...
                    data_schema=self._modifiers_json_schema(default=user_input.get(CONF_ADVANCED_ROOMS_JSON, "")),
                    errors={CONF_ADVANCED_ROOMS_JSON: "invalid_json"},
                )
            self._options[CONF_MODIFIERS] = self._modifiers
            return await self.async_step_modifiers_menu()

        return self.async_show_form(step_id="modifiers_json", data_schema=self._modifiers_json_schema())
        ...
        return vol.Schema(
            {
                vol.Optional(
                    CONF_ADVANCED_ROOMS_JSON,
                    default=default or json.dumps(self._rooms, indent=2, ensure_ascii=False),
    ...
    def _room_advanced_schema(self, default: str = "") -> vol.Schema:
        return vol.Schema(
            {
                vol.Optional(CONF_ROOM_ADVANCED_JSON, default=default): selector.TextSelector(
                    selector.TextSelectorConfig(type=selector.TextSelectorType.TEXT, multiline=True)
                ),
            }
        )
        ...
        return vol.Schema(
            {
                vol.Optional(
                    CONF_ADVANCED_ROOMS_JSON,
                    default=default or json.dumps(self._modes, indent=2, ensure_ascii=False),
                ): selector.TextSelector(
                    selector.TextSelectorConfig(type=selector.TextSelectorType.TEXT, multiline=True)
                ),
            }
    ...
    def _mode_advanced_schema(self, default: str = "") -> vol.Schema:
        return vol.Schema(
            {
                vol.Optional(CONF_MODE_ADVANCED_JSON, default=default): selector.TextSelector(
                    selector.TextSelectorConfig(type=selector.TextSelectorType.TEXT, multiline=True)
                ),
            }
        )
        ...
        return vol.Schema(
            {
                vol.Optional(
                    CONF_ADVANCED_ROOMS_JSON,
                    default=default or json.dumps(self._modifiers, indent=2, ensure_ascii=False),
                ): selector.TextSelector(
                    selector.TextSelectorConfig(type=selector.TextSelectorType.TEXT, multiline=True)
                ),
            }
    ...
    def _modifier_advanced_schema(self, default: str = "") -> vol.Schema:
        return vol.Schema(
            {
                vol.Optional(CONF_MODIFIER_ADVANCED_JSON, default=default): selector.TextSelector(
                    selector.TextSelectorConfig(type=selector.TextSelectorType.TEXT, multiline=True)
                ),
            }
        )
